...
alter table insarok change column name irum varchar(30);
-- irum필드를 name필드로 수정하시오? 길이는 가변문자형(20)
alter table insarok change column irum name varchar(20);
...
select * from insarok where gender='남자' and age<=30;
...
select * from insarok where buser='영업과' and gender='남자';
...
select name as '성명',gender as '성별',age as '나이',address as '주소' from insarok where gender='여자' and age between 20 and 29;
...
update insarok set jikwi='부장' where name='홍길동' and buser='인사과';
...
-- 영업과 사원을 출력?
select * from insarok where buser='영업과' and jikwi='사원';
...
select * from insarok where name='홍길동';
delete from insarok where name='홍길동' and buser='영업과';

-- 여자중에서 부산에 사는 사람을 삭제하시오.?
select * from insarok where gender='여자' and address='부산';
